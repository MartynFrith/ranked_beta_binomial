...
selection = ct_selection.loc[:, gen_mask] #This is the xarray that we need to take forward!

# selection.to_netcdf("K_matrix_hg19_10rds_full.nc")
...
entropy_mask = np.isin(np.arange(entropy_rank.shape[0]), sel) #Entropy masked
...
if median_or_mode == 'median':
    K_matrix =  xr.DataArray(beta_distribution.median(1 + m, 1 + (r - m)), #I changed to median so each version of K matrix has only one probability at each site in
                             dims = ('ct', 'gen'), 
    ...
    b = 1+(r-m)
    beta_mode = (a-1)/(a+b-2)
    K_matrix =  xr.DataArray(beta_mode, #I changed to median so each version of K matrix has only one probability at each site in
                             dims = ('ct', 'gen'), 
                             coords = {'ct': selection.coords['ct'],
                                       'gen':selection.coords['gen']})
    
    sampled_K = K_matrix[:, entropy_mask] #Read depth and entropy ranked site filtered K matrix (sampled K)
    sampled_K.to_netcdf("K_matrices/K_matrix_mode_beta_selection/K_matrix_hg38_{}_{}rds_mode.nc".format(n_sites, min_rd)) #Sampled K saved
else:
